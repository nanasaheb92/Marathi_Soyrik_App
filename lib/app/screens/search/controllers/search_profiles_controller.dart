...
class SearchProfilesController extends GetxController {
  ...
  List<String> drinkingHabits = ["Occasionally", "Yes", "No"];
  List<String> smokingHabits = ["Occasionally", "Yes", "No"];
...
//ADVANCE SEARCH
  Rx<Search> searchProfile = Search(gender: "Female").obs;
...
//SEARCH BY ID
  void getSearchMatchesById(bool reinit) async {
    List<MatchProfile> emptyList = [];
    if (reinit) {
      page.value = 1;
      totalPage.value = 1;
    ...
    showSearchResults.value = true;
    await _settingsRepository
        .searchProfilesById(searchProfile.value.id ?? "", page.value)
        .then((value) {
      isLoading.value = false;
      if (value.status == Status.COMPLETED) {
        totalPage.value = (value.data["total_pages"] ?? "") == ""
