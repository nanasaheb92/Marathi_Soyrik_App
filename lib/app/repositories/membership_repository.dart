...
class MembershipRepository {
  ...
  Future<ApiResponse> saveProfileVisibilityStatus(var data) async {
    log(data.toString());
    var body = dio.FormData.fromMap(data);

    return await apiProvider.makeAPICall("POST", "update-visibility-status.php", body).then((value) {
      value.data = Status.COMPLETED == value.status;
      return value;
    });
  }

  Future<ApiResponse> fetchProfileVisibilityStatus(var data) async {
    var body = dio.FormData.fromMap(data);
    return await apiProvider.makeAPICall("POST", "update-visibility-status.php", body).then((value) {
      log(value.data.toString());
      if (value.status == Status.COMPLETED) {
        value.data = ProfileSettings.fromJson(value.data["data"]);
      } else {
        value.data = ProfileSettings();
      }
      return value;
    });
  }
  ...
  Future<ApiResponse> saveIdProof(var data) async {
    ...
    if (data["id_proof"] != "") {
      body.files.add(
        MapEntry(
          "file",
          await dio.MultipartFile.fromFile(data["id_proof"], filename: data["id_proof"].toString().split("/").last),
        ),
      );
    }

    return await apiProvider.makeAPICall("POST", "upload-document.php", body).then((value) {
      value.data = Status.COMPLETED == value.status;
      return value;
    });
  }
  ...
  Future<ApiResponse> saveProfilePic(var data) async {
    ...
    var body = dio.FormData.fromMap(data);
    body.fields.removeWhere((element) => element.key == "photo1");
    body.fields.removeWhere((element) => element.key == "photo2");
    body.fields.removeWhere((element) => element.key == "photo3");
    body.fields.removeWhere((element) => element.key == "photo4");

    if (data["photo1"] != "") {
      body.files.add(
        MapEntry(
          "photo1",
          await dio.MultipartFile.fromFile(data["photo1"], filename: data["photo1"].toString().split("/").last),
        ),
      );
    }
    ...
    if (data["photo2"] != "") {
      body.files.add(
        MapEntry(
          "photo2",
          await dio.MultipartFile.fromFile(data["photo2"], filename: data["photo2"].toString().split("/").last),
        ),
      );
    }
    ...
    if (data["photo3"] != "") {
      body.files.add(
        MapEntry(
          "photo3",
          await dio.MultipartFile.fromFile(data["photo3"], filename: data["photo3"].toString().split("/").last),
        ),
      );
    }
    ...
    if (data["photo4"] != "") {
      body.files.add(
        MapEntry(
          "photo4",
          await dio.MultipartFile.fromFile(data["photo4"], filename: data["photo4"].toString().split("/").last),
        ),
      );
    }

    return await apiProvider.makeAPICall("POST", "upload-other-pictures.php", body).then((value) {
      value.data = Status.COMPLETED == value.status;
      return value;
    });
  }
